...
text2color = {"Positive": "green", "Neutral": "violet", "Negative": "red"}


def display_headings():
    st.set_page_config(page_title="FPT News Hub", layout="wide")
...
def display_mention_statistics(data: pd.DataFrame):
    ...
    data.sort_values(
        by="week", ascending=True, inplace=True, ignore_index=True
    )
...
def display_wordcloud(df_recent, n=10):
    ...
    stopwords = open(
        "Preprocess/vietnamese-stopwords.txt", "r", encoding="UTF-8"
    )
